...
def prepare_dataset(
    ...
    # wav_list = glob.glob(f'{data_folder}/**/*.wav')

    # # train_folder = glob.glob(f'{data_folder}/**')
    ...
    # split dataset
    spk_ids =  [f.split('/')[-1] for f in glob.glob(f'{data_folder}/**') if os.path.isdir(f)]
    
    train_list=[]
    valid_list=[]
    test_list=[]
    for spk_id in tqdm(spk_ids):
        wav_files = glob.glob(f'{data_folder}/{spk_id}/*.wav')
        
        # 데이터 10개 이상인 것만 valid, test 데이터로 사용
        if len(wav_files) >= 10:
            train_list += random.sample(wav_files, int(len(wav_files)*split_ratio[0]/100))
            valid_list += random.sample(wav_files, int(len(wav_files)*split_ratio[1]/100))
            test_list += random.sample(wav_files, int(len(wav_files)*split_ratio[2]/100))
        else:
            train_list += wav_files

    # Creating json files
    create_json(train_list, save_json_train)
    create_json(valid_list, save_json_valid)
    create_json(test_list, save_json_test)
